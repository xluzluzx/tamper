...
// Main tamper function that surrounds the query with a zero-versioned comment for MySQL
function tamper(payload) {
  ...
  // If there is a space in the payload, apply the zero-versioned comment
  if (payload.includes(' ')) {
    const zeroVersionedComment = `/*!00000${payload.slice(payload.indexOf(' ') + 1)}*/`;
    retVal = `${payload.slice(0, payload.indexOf(' '))} ${zeroVersionedComment}${postfix}`;
  }
